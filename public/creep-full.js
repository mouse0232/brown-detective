...
`;
    ...
    async function getBestWorkerScope() {
        try {
            ...
            // 根据当前加载的脚本确定 worker 脚本名称
            const currentScriptSrc = document?.currentScript?.src || window.location.pathname;
            const scriptName = currentScriptSrc.includes('creep-lite') ? 'creep-lite.js' : 'creep-full.js';
            const scriptSource = `./${scriptName}`;
    ...
    */
    ...
    async function getScriptSize() {
        ...
        try {
            // @ts-expect-error if unsupported
            url = document?.currentScript?.src || (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('creep-full.js', document.baseURI).href);
        }
        catch (err) { }
        if (!url)
            return null;

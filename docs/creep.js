...
// ============ 添加：API 自动上报功能 ============
(async function sendToAntiBotAPI() {
  const API_ENDPOINT = '/api/fingerprint'
  
  // 等待指纹采集完成
  await new Promise(resolve => {
    const checkFP = setInterval(() => {
      if (window.Creep && typeof window.Creep === 'function') {
        clearInterval(checkFP)
        resolve()
      }
    }, 100)
    setTimeout(() => { clearInterval(checkFP); resolve() }, 5000)
  })
  
  // 采集指纹并发送
  try {
    const fingerprint = await window.Creep({ cacheEnabled: true, cacheTTL: 1800000 })
    ...
    console.log('[Anti-Bot API] 分析结果:', result.analysis)
  } catch (e) {
    console.log('[Anti-Bot API]', e.message)
  }
})()

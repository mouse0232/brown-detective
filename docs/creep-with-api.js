...
(function() {
  ...
  // 等待指纹数据就绪（CreepJS 会把结果存到 window.fingerprintData）
  setTimeout(async function sendToAPI() {
    try {
      // 从页面提取指纹数据（CreepJS 已经计算好的）
      const fpData = {
        // 使用 CreepJS 暴露的内部变量
        navigator: window.navigator,
        screen: window.screen,
        // 如果需要更详细的数据，可以监听 CreepJS 的输出
      }
